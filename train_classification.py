...
def _main_(args):
    ...
    dataset_train_path = os.path.join(args.folder, "train")
    dataset_val_path = os.path.join(args.folder, "val")

    for folder in [dataset_train_path, dataset_val_path]:
        if not os.path.isdir(folder):
            raise Exception("{} doesn't exist!".format(folder))

    classes_train = next(os.walk(dataset_train_path))[1]
    classes_val = next(os.walk(dataset_val_path))[1]

    if not classes_val == classes_train:
        raise Exception("The training and validation classes must be the same!")
    else:
        folders = classes_train
    ...
    depth = 3 if not config['model']['gray_mode'] else 1
    ...
    for folder in folders:
        images_train_paths += list(list_images(os.path.join(dataset_train_path, folder)))
        images_val_paths += list(list_images(os.path.join(dataset_val_path, folder)))
    ...
    check_point_saver_best = ModelCheckpoint(model_name, monitor='val_acc', verbose=1,
                                             save_best_only=True, save_weights_only=False, mode='auto', period=1)
    ckp_model_name = os.path.splitext(model_name)[1] + "_ckp.h5"
    check_point_saver = ModelCheckpoint(ckp_model_name, verbose=1,
                                        save_best_only=False, save_weights_only=False, period=10)

    tb = TensorBoard(log_dir=config['train']['tensorboard_log_dir'], histogram_freq=0, batch_size=batch_size,
                     write_graph=True,
    ...
    train_generator = BatchGenerator(images_train_paths,
                                     generator_config,
                                     norm=feature_extractor.normalize,
                                     jitter=True)
    val_generator = BatchGenerator(images_val_paths,
                                   generator_config,
                                   norm=feature_extractor.normalize,
                                   jitter=False)
    ...
    model.fit_generator(
        train_generator,
        steps_per_epoch=len(images_train_paths) // batch_size,
        epochs=epochs,
        validation_data=val_generator,
        validation_steps=len(images_val_paths) // batch_size,
        callbacks=[check_point_saver_best, check_point_saver, tb],
        workers=12,
        max_queue_size=40)
